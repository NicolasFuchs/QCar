...
public class WorldManagerPhysicsHelper {
  ...
  public static List<ICollision> computeCollisions(List<IQCar> drivenQCars, List<IDecision> allDecisions, List<IQCar> allQCars) {
    ...
    for (int i = 0; i < drivenQCars.size(); i++) {
      ...
      if (requestedTranslation == 0 || (!isAngleMovement && (((sideId == 0 || sideId == 1) && requestedTranslation > 0) || ((sideId == 2 || sideId == 3) && requestedTranslation < 0)))) continue; // Aucune collision g�n�r�e car mouvemenent interne au QCar
      Line2D[] A1A2 = findAxes(decision, car);
      double A2_coor = Double.MAX_VALUE;    // La collision "finale" est celle dont la coordonn�e A2 est la plus petite
      colList.add(null);
      ...
      for (int j = 0; j < allQCars.size(); j++) {
        ...
        for (int k = 0; k < 4; k++) {
          ...
          if (PointInA.getX() >= 0 && PointInA.getX() <= 1 && PointInA.getY() >= 0 && PointInA.getY() <= 1) { // cas o� le point est dans la zone balay�e (y compris les limites)
  ...
  // retourne l'ID du vertex/side du QCar dirig� impliqu� dans la collision
  ...
  // retourne 2 (angleMovement) ou 3 (!angleMovement) limites d�limitant l'aire balay�e par le mouvement
...
//      }
  ...
  // retourne un Point si une intersection est trouv�e, sinon null
  ...
  private static Point2D findIntersection(Line2D seg1, Line2D seg2) {
    if (seg1.intersectsLine(seg2)) {
      ...
      // 2 droites constantes superpos�es
