    ...
    for (int i = 0; i < drivenQCars.size(); i++) {
      ...
      for (int j = 0; j < allQCars.size(); j++) {
        ...
        for (int k = 0; k < 4; k++) {
          ...
          if (PointInA.getX() > 0 && PointInA.getX() <= 1 && PointInA.getY() > 0 && PointInA.getY() <= 1) { // cas où le point est dans la zone balayée
...
//              System.out.println("Point in the swept area  -->  X : " + hitCar.vertex(k).getX() + " Y : " + hitCar.vertex(k).getY());
            }
          }
  ...
  private static Line2D[] findAxes(IDecision decision, IQCar car) {
    ...
    if (decision.isAngleMovement()) {
      p1 = car.vertex((sideId+1)%4);
      p2 = car.vertex((sideId+2)%4);
      if (((sideId == 0 || sideId == 3) && decision.requestedTranslation() > 0) || ((sideId == 1 || sideId == 2) && decision.requestedTranslation() < 0)) {
        p3 = car.vertex((sideId+1)%4);
        p4 = car.vertex(sideId);
      } else {
        p3 = car.vertex(sideId);
        p4 = car.vertex((sideId+1)%4);
      }
    } else {
      p1 = car.vertex(sideId);
      p2 = car.vertex((sideId+1)%4);
      p3 = car.vertex((sideId+2)%4);
      p4 = car.vertex((sideId+1)%4);
    }
  ...
  // retourne 2 (angleMovement) ou 3 (!angleMovement) Lines délimitant l'aire balayée par le mouvement
  private static Line2D[] findLines(IDecision decision, IQCar car, Line2D A2) {
    ...
    if (decision.isAngleMovement()) {
      ...
      if (p1.getX() >= p2.getX()) res[2] = new Line2D.Double(p2.getX()+shiftX, p2.getY()+shiftY, p1.getX()+shiftX, p1.getY()+shiftY);
      else res[2] = new Line2D.Double(p1.getX()+shiftX, p1.getY()+shiftY, p2.getX()+shiftX, p2.getY()+shiftY);
    }
    return res;
  }
  ...
  private static Point2D findIntersection(Line2D seg1, Line2D seg2) {
    if (seg1.intersectsLine(seg2)) {
      ...
      if (seg1.getX1() == seg1.getX2() && seg2.getY1() == seg2.getY2()) {//(seg1.getX1() >= seg2.getX1() && seg1.getX1() <= seg2.getX2()) && (seg2.getY1() >= seg1.getY1() && seg2.getY1() <= seg1.getY2())) { // cas d'une ligne horizontale et d'une ligne verticale
        return new Point2D.Double(seg1.getX1(), seg2.getY1());
      }
      if (seg1.getY1() == seg1.getY2() && seg2.getX1() == seg2.getX2()) {//(seg1.getY1() >= seg2.getY1() && seg1.getY1() <= seg2.getY2()) && (seg2.getX1() >= seg1.getX1() && seg2.getX1() <= seg1.getX2())) { // cas d'une ligne verticale et d'une ligne horizontale
...
//        if (seg1.getX2() <= Math.max(seg2.getX1(), seg2.getX2()) && seg1.getX2() >= Math.min(seg2.getX1(), seg2.getX2())) return new Point2D.Double(seg1.getX2(),seg1.getY2());
      ...
      if (seg1.getX1() == seg1.getX2() || seg1.getY1() == seg1.getY2()) {
        if (seg1.getX1() == seg1.getX2()) {
          ...
          /*if (y <= Math.max(seg1.getY1(), seg1.getY2()) && y >= Math.min(seg1.getY1(), seg1.getY2()))*/ return new Point2D.Double(seg1.getX1(), y);
        }
        if (seg1.getY1() == seg1.getY2()) {
          double x = (seg1.getY1()-d2)/m2;
          /*if (x <= Math.max(seg1.getX1(), seg1.getX2()) && x >= Math.min(seg1.getX1(), seg1.getX2()))*/ return new Point2D.Double(x, seg1.getY1());
        }
      } else if (seg2.getX1() == seg2.getX2() || seg2.getY1() == seg2.getY2()) {
        if (seg2.getX1() == seg2.getX2()) {
          double y = m1*seg2.getX1()+d1;
          /*if (y <= Math.max(seg2.getY1(), seg2.getY2()) && y >= Math.min(seg2.getY1(), seg2.getY2()))*/ return new Point2D.Double(seg2.getX1(), y);
        }
        if (seg2.getY1() == seg2.getY2()) {
          double x = (seg2.getY1()-d1)/m1;
          /*if (x <= Math.max(seg2.getX1(), seg2.getX2()) && x >= Math.min(seg2.getX1(), seg2.getX2()))*/ return new Point2D.Double(x, seg2.getY1());
        }
      // 2 droites quelconques
      } else {
        double x = (d1-d2)/(m2-m1);
        double y = m1*x + d1;
        /*if (x <= Math.max(seg1.getX1(), seg1.getX2()) && x >= Math.min(seg1.getX1(), seg1.getX2()))*/ return new Point2D.Double(x,y);
      }
    }
    return null;
  }
